<template>
  <div class="bg-success">
      ...
      <h1>Datos del componentHijo: {{cadena2}}</h1>
      <h1>Datos de componentHijo: {{cadena3}}</h1>
      <ul  >
           <li v-for="(c, index ) in cadena3" :key="index" > 
               <h1>{{c.modelo}}</h1>
               <br>
      ...
      <componentHijo v-bind:mensaje= "nombre" :apellido="apellido" :edad="edad" :mensaje1="lista_peliculas" :priori="prioridad" @cad="cadena2 = $event" :coches="lista_coches" @coche="cadena3 = $event"/>
  </div>
</template>

<script>
import componentHijo from './Hijo.vue'
export default {
    props: ['mensaje1'], 
    components:{
       componentHijo
